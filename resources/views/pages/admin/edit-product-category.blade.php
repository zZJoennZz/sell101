                    ...
                    <div class="card-content" style="padding-bottom:0;">
                        <div class="row valign-wrapper" style="margin-bottom: 24px;">
                            ...
                            <div class="col s9 m10" style="padding-left:0;">
                                <span class="card-title" style="margin-bottom:0; font-size:1.5rem;">Edit Product Category</span>
                                <p class="grey-text" style="font-size:1rem;">Update the details of this category.</p>
                            </div>
                        </div>
                        @if(session('success'))
                        ...
                        @endif
                        <form id="edit-category-form" method="POST" action="{{ route('admin.updateproductcategory', $category->id) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row" style="margin-bottom:0;">
                                <div class="input-field col s12" style="margin-bottom:8px;">
                                    <input id="name" name="name" type="text" class="validate @error('name') invalid @enderror" value="{{ old('name', $category->name) }}" required>
                                    <label for="name" @if(old('name', $category->name)) class="active" @endif>Name</label>
                                    <span class="helper-text" data-error="Required" data-success="">Category name (e.g. Pain Relief)</span>
                                </div>
                                <div class="input-field col s12" style="margin-bottom:8px;">
                                    <input id="slug" name="slug" type="text" class="validate @error('slug') invalid @enderror" value="{{ old('slug', $category->slug) }}" required>
                                    <label for="slug" @if(old('slug', $category->slug)) class="active" @endif>Slug</label>
                                    <span class="helper-text" data-error="Required" data-success="">URL-friendly identifier (e.g. pain-relief)</span>
                                </div>
                                <div class="input-field col s12" style="margin-bottom:8px;">
                                    <textarea id="description" name="description" class="materialize-textarea @error('description') invalid @enderror">{{ old('description', $category->description) }}</textarea>
                                    <label for="description" @if(old('description', $category->description)) class="active" @endif>Description</label>
                                ...
                                <div class="input-field col s12" style="margin-bottom:8px;">
                                    <select id="status" name="status" required class="@error('status') invalid @enderror">
                                        <option value="" disabled {{ old('status', $category->status) ? '' : 'selected' }}>Choose status</option>
                                ...
                                <div class="col s12 right-align" style="margin-top:16px; margin-bottom:16px;">
                                    <button class="btn waves-effect waves-light" type="submit" style="width:100%;max-width:220px;">Update Category
